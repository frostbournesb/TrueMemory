...
- **Novelty** is vector-similarity inversion via neuromem's hybrid
...
- **Salience** delegates to neuromem's existing
...
- **Prediction error** delegates to neuromem's existing
...
The delegation to `neuromem.salience` and `neuromem.predictive` is
intentional: those modules already implement the surprise and salience
scoring that neuromem-core uses for retrieval weighting. Reusing them
keeps the encoding gate consistent with the retrieval layer and avoids
code duplication. If either module is unavailable (older neuromem-core
...
# Try to import neuromem's existing scoring modules. If they're not
# available (older neuromem-core or partial install), fall back to
# internal heuristics.
_HAS_NEUROMEM_SALIENCE = False
_HAS_NEUROMEM_PREDICTIVE = False
try:
    from neuromem.salience import compute_message_salience as _nm_salience
    _HAS_NEUROMEM_SALIENCE = True
except ImportError:
    _nm_salience = None

try:
    from neuromem.predictive import compute_surprise_score as _nm_surprise
    from neuromem.predictive import extract_facts as _nm_extract_facts
    _HAS_NEUROMEM_PREDICTIVE = True
except ImportError:
    _nm_surprise = None
...
# whether the delegation to neuromem-core's scoring was active or not.
if not _HAS_NEUROMEM_SALIENCE:
    log.warning(
        "neuromem.salience.compute_message_salience not available; "
        "using fallback salience heuristic. Install/upgrade neuromem-core "
        "for the full salience signal."
    )
if not _HAS_NEUROMEM_PREDICTIVE:
    log.warning(
        "neuromem.predictive.compute_surprise_score not available; "
        "using fallback prediction-error heuristic. Install/upgrade "
        "neuromem-core for the full prediction-error signal."
    )
...
class EncodingGate:
    ...
    are filtered out. All three signals delegate to neuromem-core modules
    where possible, so the gate stays consistent with retrieval behavior.

    Args:
        memory: A neuromem Memory instance for searching existing memories.
    ...
    def evaluate(self, fact: str, category: str = "") -> EncodingDecision:
        ...
        if _HAS_NEUROMEM_PREDICTIVE and _nm_extract_facts is not None:
            try:
    ...
    def _compute_novelty(self, fact: str) -> float:
        ...
        Uses neuromem's hybrid search to find the most similar existing
        ...
        # neuromem search returns results with a `score` field (engine.py:726)
    ...
    # ------------------------------------------------------------------
    # Signal 2: Salience — delegates to neuromem.salience when available
    # ------------------------------------------------------------------

    def _compute_salience(self, fact: str, category: str = "") -> float:
        """
        Salience score combining neuromem's built-in salience with a
        category-based boost from the LLM extractor.

        If `neuromem.salience.compute_message_salience` is available,
        we use it as the base signal — this ensures the encoding gate
        agrees with neuromem's own salience layer used during retrieval.
        Otherwise we fall back to a minimal heuristic.
        """
        # Base salience from neuromem-core (handles length, numbers, dates,
        # emotional markers, life events, ALL-CAPS, etc.) — see
        # neuromem/salience.py compute_message_salience
        # We pass modality="chat" because ingestion captures conversational
        # facts; neuromem's salience weights other modalities (email, ocr,
        # calendar, etc.) differently. This preserves that signal.
        if _HAS_NEUROMEM_SALIENCE and _nm_salience is not None:
            try:
                base = float(_nm_salience(fact, "chat"))
            except Exception as e:
                log.debug("neuromem salience failed, using fallback: %s", e)
    ...
    def _fallback_salience(fact: str) -> float:
        """Minimal heuristic salience when neuromem-core isn't available."""
        if not fact:
            return 0.0
        length = len(fact)
        if length < 10:
    ...
    # ------------------------------------------------------------------
    # Signal 3: Prediction error — delegates to neuromem.predictive
    # ------------------------------------------------------------------
    ...
    def _compute_prediction_error(self, fact: str, novelty: float) -> float:
        ...
        Delegates to neuromem's `predictive.compute_surprise_score` when
        ...
        # Use neuromem's surprise score if available
        if _HAS_NEUROMEM_PREDICTIVE and _nm_surprise is not None:
            try:
                ...
                log.debug("neuromem surprise failed, using fallback: %s", e)
    ...
    def _fallback_prediction_error(self, fact: str, novelty: float) -> float:
        """Fallback prediction error when neuromem.predictive isn't available."""
