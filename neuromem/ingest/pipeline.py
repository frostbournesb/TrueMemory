...
from neuromem.ingest.extractor import extract_facts, ExtractedFact, extract_facts_simple
from neuromem.ingest.encoding_gate import EncodingGate, EncodingDecision
from neuromem.ingest.dedup import check_duplicate, DedupAction
...
_LOCK_PATH = Path(os.environ.get(
    "NEUROMEM_INGEST_LOCK",
    str(Path.home() / ".neuromem" / "ingest.lock"),
))
...
def _set_busy_timeout(memory, timeout_ms: int = 10_000) -> None:
    ...
    Older neuromem-core versions may not expose ``_engine`` / ``conn``.
...
class IngestionPipeline:
    ...
    Args:
        memory: A neuromem Memory instance.
    ...
    def _store_fact(
        ...
        """Store a new fact in neuromem.

        Note: neuromem's client.Memory.add() currently treats the `metadata`
