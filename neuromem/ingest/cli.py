"""
CLI entry point for neuromem-ingest.

Usage:
    neuromem-ingest /path/to/transcript.json --user alice
    neuromem-ingest --install   # Install hooks into Claude Code settings
"""
...
from neuromem.ingest import ingest, save_trace, IngestionResult
from neuromem.ingest.models import LLMConfig, hydrate_config


def main():
    parser = argparse.ArgumentParser(
        description="Neuromem Ingestion — biomimetic memory encoding",
        prog="neuromem-ingest",
    )
    ...
    p_ingest.add_argument("--db", default=None, help="Path to neuromem database")
    ...
    p_install.add_argument("--db", default=None, help="Path to neuromem database")
    ...
    # --- uninstall command ---
    p_uninstall = sub.add_parser("uninstall", help="Remove neuromem-ingest hooks from Claude Code settings")
    p_uninstall.add_argument("--dry-run", action="store_true", help="Show what would be removed without writing")

    # --- logs command ---
    p_logs = sub.add_parser("logs", help="Tail recent hook log files from ~/.neuromem/logs/")
...
def _run_ingest(args):
    ...
    # Preflight: verify neuromem-core is importable before we try to
    ...
    try:
        import neuromem  # noqa: F401
    except ImportError:
        print(
            "ERROR: neuromem-core is not installed.\n"
            "       Install with: pip install neuromem-core\n"
            "       (neuromem-ingest depends on neuromem-core>=0.1.3)",
            file=sys.stderr,
        )
        sys.exit(2)
    ...
    if args.provider != "auto":
        ...
        if config.provider == "claude_cli":
            from neuromem.ingest.models import _claude_cli_available
...
def _preflight_writable_target(target: str | None, *, kind: str) -> bool:
    ...
    if not target:
        ...
        # be used, e.g. ~/.neuromem/memories.db which neuromem-core owns).
        return True
    ...
    probe = parent / f".neuromem.ingest_preflight_{os.getpid()}"
    try:
...
_NEUROMEM_CONFIG_PATH = Path.home() / ".neuromem" / "config.json"


def _load_neuromem_config() -> dict:
    """Load persistent config from ~/.neuromem/config.json."""
    if _NEUROMEM_CONFIG_PATH.exists():
        try:
            return json.loads(_NEUROMEM_CONFIG_PATH.read_text())
        except (json.JSONDecodeError, OSError):
            return {}
    return {}


def _save_neuromem_config(config: dict) -> None:
    """Save config to ~/.neuromem/config.json."""
    _NEUROMEM_CONFIG_PATH.parent.mkdir(parents=True, exist_ok=True)
    _NEUROMEM_CONFIG_PATH.parent.chmod(0o700)
    _NEUROMEM_CONFIG_PATH.write_text(json.dumps(config, indent=2))
    _NEUROMEM_CONFIG_PATH.chmod(0o600)


def _run_setup(args):
    """Interactive first-time setup wizard for Neuromem."""
    print(_SAURON_BANNER)
    print("Welcome to Neuromem setup! Let's get you configured.\n")

    config = _load_neuromem_config()
    ...
    if tier == "pro":
        try:
            ...
            print("  \033[33m⚠ Pro tier requires: pip install neuromem-core[gpu]\033[0m")
            if not args.non_interactive:
                do_install = input("  Install now? [y/N]: ").strip().lower()
                if do_install == "y":
                    import subprocess
                    subprocess.run([sys.executable, "-m", "pip", "install", "neuromem-core[gpu]"])
                else:
    ...
    try:
        ...
        from neuromem.vector_search import set_embedding_model, get_model
    ...
    _save_neuromem_config(config)
    print()
    print(f"  \033[32m✓ Config saved to {_NEUROMEM_CONFIG_PATH}\033[0m")
    ...
    print()
    print("  Run \033[1mneuromem-ingest status\033[0m to verify everything.")
    print()
    print("  \033[2mThanks for using Neuromem, a Sauron company.\033[0m")
    print()
...
def _run_install(args):
    ...
    if isinstance(_legacy_compact, list):
        _cleaned = [
            ...
            if not (
                isinstance(h, dict)
                and "neuromem" in str(h.get("command", "")).lower()
            )
        ]
        if _cleaned:
    ...
    # how to use neuromem's MCP tools during conversations.
    #
    ...
    print(f"\nTo verify: neuromem-ingest status")


_CLAUDE_MD_MARKER_START = "<!-- BEGIN neuromem-ingest managed section -->"
_CLAUDE_MD_MARKER_END = "<!-- END neuromem-ingest managed section -->"
...
def _merge_claude_md(template_path: Path, target_path: Path) -> None:
    ...
    try:
        ...
        print(f"  [OK] Merged neuromem instructions into {target_path}")
    except OSError as e:
...
def _run_status(args):
    ...
    print("Neuromem Ingestion — Status Check")
    print("=" * 40)

    # 1. neuromem-core availability
    try:
        import neuromem
        version = getattr(neuromem, "__version__", "unknown")
        print(f"  [OK] neuromem-core {version} importable")
    except ImportError as e:
        print(f"  [FAIL] neuromem-core not importable: {e}")
        print("         Install with: pip install neuromem-core")
        return

    # 2. LLM backend
    try:
        from neuromem.ingest.models import auto_detect
        cfg = auto_detect()
    ...
    if settings_path.exists():
        try:
            ...
            for event in expected:
                ...
                has_neuromem = any(
                    "neuromem" in str(h.get("command", "")).lower()
            ...
            if missing:
                ...
                print("         Run: neuromem-ingest install")
    ...
    #
    # neuromem.Memory exposes stats() which returns a dict including
    ...
    trace_dir = Path.home() / ".neuromem" / "traces"
    log_dir = Path.home() / ".neuromem" / "logs"
    if trace_dir.exists():
...
def _run_uninstall(args):
    """Remove neuromem hooks from Claude Code settings."""
    settings_path = Path.home() / ".claude" / "settings.json"
    ...
    for event in list(hooks.keys()):
        ...
        kept = []
        for h in event_hooks:
            if isinstance(h, dict) and "neuromem" in str(h.get("command", "")).lower():
    ...
    if not removed:
        print("No neuromem hooks found in settings.json")
        return
    ...
    print(f"Removed {len(removed)} neuromem hooks from {settings_path}")
    for r in removed:
        print(f"  - {r}")
    ...
    if claude_md_path.exists():
        try:
            ...
            if _CLAUDE_MD_MARKER_START in content and _CLAUDE_MD_MARKER_END in content:
                ...
                print(f"Removed neuromem-managed section from {claude_md_path}")
        except OSError:
            pass
...
_TRACE_DIR = Path.home() / ".neuromem" / "traces"
_LOG_DIR = Path.home() / ".neuromem" / "logs"
...
def _sanitize_session(session: str) -> str:
    ...
    ``~/.neuromem/traces`` and ``~/.neuromem/logs``. A value like
...
def _run_logs(args):
    """Tail recent hook log output from ~/.neuromem/logs/."""
