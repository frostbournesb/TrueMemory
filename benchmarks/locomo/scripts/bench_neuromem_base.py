#!/usr/bin/env python3
"""
LoCoMo Benchmark — Neuromem Base Tier
======================================
Neuromem Base tier using Model2Vec (potion-base-8M, 256-dim) embeddings
...
Dependencies: neuromem-core, sentence-transformers
...
Usage:
    ...
    modal run --detach bench_neuromem_base.py          # Full run (10 convs, 1540 Qs)
    modal run --detach bench_neuromem_base.py --smoke  # Smoke test (1 conv, 5 Qs)
...
app = modal.App("locomo-neuromem-base")
...
def _bench_conv(conv_data, conv_idx, smoke=False):
    ...
    print(f"  [neuromem_base] Conv {conv_idx} ({sid}): {len(parse_conv(conv_data))} msgs, {n_qs} Qs")

    # Step 1: Retrieve
    t0 = time.time()
    try:
        all_retrieved = retrieve_neuromem_base(conv_data, conv_idx)
    except Exception as e:
...
def orchestrate(locomo_data: list, smoke: bool = False):
    """Run Neuromem Base, checkpointing after each conversation."""
    system = "neuromem_base"
...
def main(smoke: bool = False, dataset: str = None):
    ...
    print(f"\n{'='*60}")
    print(f"LoCoMo Benchmark — Neuromem Base — {'SMOKE TEST' if smoke else 'FULL RUN'}")
    print(f"{'='*60}")
