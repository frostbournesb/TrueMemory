...
app = modal.App("locomo-truememory-pro")
...
def _bench_conv(conv_data, conv_idx, smoke=False):
    ...
    print(f"  [truememory_pro] Conv {conv_idx} ({sid}): {len(parse_conv(conv_data))} msgs, {n_qs} Qs")

    # Step 1: Retrieve
    t0 = time.time()
    try:
        all_retrieved = retrieve_truememory_pro(conv_data, conv_idx)
    except Exception as e:
...
def orchestrate(locomo_data: list, smoke: bool = False):
    """Run TrueMemory Pro, checkpointing after each conversation."""
    system = "truememory_pro"
