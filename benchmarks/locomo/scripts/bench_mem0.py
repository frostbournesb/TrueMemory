#!/usr/bin/env python3
"""
LoCoMo Benchmark — Mem0
========================
Mem0 (mem0ai) memory system with LLM-based memory extraction.
Uses OpenRouter as LLM backend and HuggingFace sentence-transformers for embeddings.
Mem0 extracts structured memories from raw text using an LLM, then retrieves via
embedding similarity.

This is a fully self-contained Modal script. No local imports required.

Dependencies: mem0ai, sentence-transformers
...
Usage:
    ...
    modal run --detach bench_mem0.py          # Full run (10 convs, 1540 Qs)
    modal run --detach bench_mem0.py --smoke  # Smoke test (1 conv, 5 Qs)
...
def _bench_conv(conv_data, conv_idx, smoke=False):
    ...
    print(f"  [mem0] Conv {conv_idx} ({sid}): {len(parse_conv(conv_data))} msgs, {n_qs} Qs")

    # Step 1: Retrieve
    t0 = time.time()
    try:
        all_retrieved = retrieve_mem0(conv_data, conv_idx)
    except Exception as e:
...
def orchestrate(locomo_data: list, smoke: bool = False):
    """Run Mem0, checkpointing after each conversation."""
    system = "mem0"
...
def main(smoke: bool = False, dataset: str = None):
    ...
    print(f"\n{'='*60}")
    print(f"LoCoMo Benchmark — Mem0 — {'SMOKE TEST' if smoke else 'FULL RUN'}")
    print(f"{'='*60}")
