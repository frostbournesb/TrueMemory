#!/usr/bin/env python3
"""
LoCoMo Benchmark — EverMemOS (Pre-built Retrieval)
====================================================
EverMemOS uses pre-computed agentic retrieval data stored on a Modal Volume.
The retrieval file (evermemos_retrieval.json) contains pre-built context for
all 1540 questions, generated by EverMemOS's BM25+Embedding+RRF+Reranker
pipeline. This script only performs answer generation and judging.

This is a fully self-contained Modal script. No local imports required.

Dependencies: openai (answer generation + judging only)
Eval: openai/gpt-4.1-mini (answers) + openai/gpt-4o-mini (judge) via OpenRouter

Pre-requisite:
    The file evermemos_retrieval.json must exist on the Modal Volume 'locomo-results'.
    Upload it with: modal volume put locomo-results evermemos_retrieval.json /

Usage:
    modal secret create openrouter-key OPENROUTER_API_KEY=sk-or-...

    modal run --detach bench_evermemos.py          # Full run (10 convs, 1540 Qs)
    modal run --detach bench_evermemos.py --smoke  # Smoke test (1 conv, 5 Qs)
...
app = modal.App("locomo-evermemos")
vol = modal.Volume.from_name("locomo-results", create_if_missing=True)
VM = "/results"
OPENROUTER_BASE_URL = "https://openrouter.ai/api/v1"

img = modal.Image.debian_slim(python_version="3.11").pip_install("openai>=1.0")
...
def _bench_conv(conv_data, conv_idx, smoke=False):
    ...
    print(f"  [evermemos] Conv {conv_idx} ({sid}): {len(parse_conv(conv_data))} msgs, {n_qs} Qs")

    # Step 1: Retrieve
    t0 = time.time()
    try:
        all_retrieved = retrieve_evermemos(conv_data, conv_idx)
    except Exception as e:
...
def orchestrate(locomo_data: list, smoke: bool = False):
    """Run EverMemOS, checkpointing after each conversation."""
    system = "evermemos"
...
def main(smoke: bool = False, dataset: str = None):
    ...
    print(f"\n{'='*60}")
    print(f"LoCoMo Benchmark — EverMemOS — {'SMOKE TEST' if smoke else 'FULL RUN'}")
    print(f"{'='*60}")
