#!/usr/bin/env python3
"""
LoCoMo Benchmark — Engram Memory
=================================
Engram (engram-core) lightweight memory system with built-in SQLite storage.
Stores facts with importance scores, retrieves via built-in search.
...
Usage:
    ...
    modal run --detach bench_engram.py          # Full run (10 convs, 1540 Qs)
    modal run --detach bench_engram.py --smoke  # Smoke test (1 conv, 5 Qs)
...
def _bench_conv(conv_data, conv_idx, smoke=False):
    ...
    print(f"  [engram] Conv {conv_idx} ({sid}): {len(parse_conv(conv_data))} msgs, {n_qs} Qs")

    # Step 1: Retrieve
    t0 = time.time()
    try:
        all_retrieved = retrieve_engram(conv_data, conv_idx)
    except Exception as e:
...
def orchestrate(locomo_data: list, smoke: bool = False):
    """Run Engram, checkpointing after each conversation."""
    system = "engram"
...
def main(smoke: bool = False, dataset: str = None):
    ...
    print(f"\n{'='*60}")
    print(f"LoCoMo Benchmark — Engram — {'SMOKE TEST' if smoke else 'FULL RUN'}")
    print(f"{'='*60}")
