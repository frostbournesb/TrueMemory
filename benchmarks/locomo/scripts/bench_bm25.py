#!/usr/bin/env python3
"""
LoCoMo Benchmark — BM25 Keyword Baseline
=========================================
BM25 (Okapi) keyword-based retrieval using rank-bm25.
No embeddings or neural models — pure TF-IDF term matching.
...
Usage:
    ...
    modal run --detach bench_bm25.py          # Full run (10 convs, 1540 Qs)
    modal run --detach bench_bm25.py --smoke  # Smoke test (1 conv, 5 Qs)
...
def _bench_conv(conv_data, conv_idx, smoke=False):
    ...
    print(f"  [bm25] Conv {conv_idx} ({sid}): {len(parse_conv(conv_data))} msgs, {n_qs} Qs")

    # Step 1: Retrieve
    t0 = time.time()
    try:
        all_retrieved = retrieve_bm25(conv_data, conv_idx)
    except Exception as e:
...
def orchestrate(locomo_data: list, smoke: bool = False):
    """Run BM25, checkpointing after each conversation."""
    system = "bm25"
...
def main(smoke: bool = False, dataset: str = None):
    ...
    print(f"\n{'='*60}")
    print(f"LoCoMo Benchmark — BM25 — {'SMOKE TEST' if smoke else 'FULL RUN'}")
    print(f"{'='*60}")
