#!/usr/bin/env python3
"""
LoCoMo Benchmark — Neuromem Pro Tier (T4 GPU)
===============================================
Neuromem Pro tier using Qwen3-Embedding-0.6B (1024-dim) embeddings,
...
Dependencies: neuromem-core[gpu], sentence-transformers
...
Usage:
    ...
    modal run --detach bench_neuromem_pro.py          # Full run (10 convs, 1540 Qs)
    modal run --detach bench_neuromem_pro.py --smoke  # Smoke test (1 conv, 5 Qs)
...
app = modal.App("locomo-neuromem-pro")
...
def _bench_conv(conv_data, conv_idx, smoke=False):
    ...
    print(f"  [neuromem_pro] Conv {conv_idx} ({sid}): {len(parse_conv(conv_data))} msgs, {n_qs} Qs")

    # Step 1: Retrieve
    t0 = time.time()
    try:
        all_retrieved = retrieve_neuromem_pro(conv_data, conv_idx)
    except Exception as e:
...
def orchestrate(locomo_data: list, smoke: bool = False):
    """Run Neuromem Pro, checkpointing after each conversation."""
    system = "neuromem_pro"
...
def main(smoke: bool = False, dataset: str = None):
    ...
    print(f"\n{'='*60}")
    print(f"LoCoMo Benchmark — Neuromem Pro (T4 GPU) — {'SMOKE TEST' if smoke else 'FULL RUN'}")
    print(f"{'='*60}")
