#!/usr/bin/env python3
"""
LoCoMo Benchmark — ChromaDB RAG Baseline
=========================================
Dense vector retrieval using ChromaDB with sentence-transformers embeddings.
Standard RAG pipeline: embed documents, cosine similarity search.
...
Usage:
    ...
    modal run --detach bench_rag.py          # Full run (10 convs, 1540 Qs)
    modal run --detach bench_rag.py --smoke  # Smoke test (1 conv, 5 Qs)
...
def _bench_conv(conv_data, conv_idx, smoke=False):
    ...
    print(f"  [rag] Conv {conv_idx} ({sid}): {len(parse_conv(conv_data))} msgs, {n_qs} Qs")

    # Step 1: Retrieve
    t0 = time.time()
    try:
        all_retrieved = retrieve_rag(conv_data, conv_idx)
    except Exception as e:
...
def orchestrate(locomo_data: list, smoke: bool = False):
    """Run RAG (ChromaDB), checkpointing after each conversation."""
    system = "rag"
...
def main(smoke: bool = False, dataset: str = None):
    ...
    print(f"\n{'='*60}")
    print(f"LoCoMo Benchmark — RAG (ChromaDB) — {'SMOKE TEST' if smoke else 'FULL RUN'}")
    print(f"{'='*60}")
